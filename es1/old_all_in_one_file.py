...
def four_means_clustering(G, K=4):
    ...
    while added < n:
        ...
        x = random.choice([el for el in G.nodes() if el not in cluster0 | cluster1 | cluster2 | cluster3 and (len(
            ...
            set(G.neighbors(el)).intersection(cluster1)) != 0 or len(
            set(G.neighbors(el)).intersection(cluster2)) != 0 or len(
            set(G.neighbors(el)).intersection(cluster3)) != 0)])
...
def spectral_clustering_parallel(G, j):
    ...
    with Parallel(n_jobs=j) as parallel:
        ...
        result = parallel(
            delayed(compute_eigen)(np.array(a), b) for a, b in double_chunks(v, nodes, math.ceil(len(G.nodes()) / j)))
    ...
    with Parallel(n_jobs=j) as parallel:
        ...
        result = parallel(
            delayed(compute_eigen)(np.array(a), b) for a, b in double_chunks(v1, nodes1, math.ceil((n1) / j)))
    ...
    with Parallel(n_jobs=j) as parallel:
        ...
        result = parallel(
            delayed(compute_eigen)(np.array(a), b) for a, b in double_chunks(v2, nodes2, math.ceil((n2) / j)))
        # Aggregates the results
    for res in result:
        c21 |= res[0]
        c22 |= res[1]
...
if __name__ == '__main__':
    ...
    four_means_clusters=four_means_clustering_opt(G)
    label=['first','second','third','fourth']
