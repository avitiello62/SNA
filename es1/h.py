...
def parallel_eigen(G,j):
    ...
    for a,b in double_chunks(v,nodes, math.ceil(len(G.nodes())/j)):
        #print(compute_eigen(a,b,math.ceil(len(G.nodes())/j)))
        print(a)
        #result=parallel(delayed(compute_eigen)(np.array(a),b,math.ceil(len(G.nodes())/j)) for a,b in double_chunks(v,nodes, math.ceil(len(G.nodes())/j)))
        #Aggregates the results
        #for res in result:
         #   print(res)
    

def compute_eigen(vec,nodes,l_vec):
    c1=set()
    c2=set()
